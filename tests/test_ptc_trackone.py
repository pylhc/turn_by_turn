...
def test_read_ptc_sci(_ptc_file_sci):
    ...
    origin = _original_trackone(track=False)
    compare_tbt(origin, new, no_binary=True)
...
def _original_trackone(track: bool = False) -> TbtData:
    ...
    matrix = [
        TransverseData(  # first "bunch"
            X=pd.DataFrame(
                index=names, data=[[0.001, -0.0003606, -0.00165823, -0.00266631]]
            ),
            Y=pd.DataFrame(
                index=names, data=[[0.001, 0.00070558, -0.00020681, -0.00093807]]
            ),
        ),
        TransverseData(  # second "bunch"
            X=pd.DataFrame(
                index=names, data=[[0.0011, -0.00039666, -0.00182406, -0.00293294]]
            ),
            Y=pd.DataFrame(
                index=names, data=[[0.0011, 0.00077614, -0.00022749, -0.00103188]]
            ),
        ),
    ]
...
def _original_simulation_data() -> TbtData:
    ...
    matrices = [
        TrackingData(  # first "bunch"
            X=pd.DataFrame(
                index=names, data=[[0.001, -0.000361, -0.001658, -0.002666]]
            ),
            ...
            E=pd.DataFrame(
                index=names, data=[[500.00088, 500.00088, 500.00088, 500.00088]]
            ),
        ),
        TrackingData(  # second "bunch"
            X=pd.DataFrame(
                index=names, data=[[0.0011, -0.000397, -0.001824, -0.002933]]
            ),
            PX=pd.DataFrame(index=names, data=[[0.0, -0.000222, -0.000405, -0.000517]]),
            Y=pd.DataFrame(
                index=names, data=[[0.0011, 0.000776, -0.000227, -0.001032]]
            ),
