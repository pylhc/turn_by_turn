...
def get_averaged_data(
    ...
    for index, bpm in enumerate(bpm_names):
        for i, _ in enumerate(matrices):
            bpm_data[index, i, : len(matrices[i][plane].loc[bpm])] = matrices[i][
                plane
            ].loc[bpm]
...
def matrices_to_array(tbt_data: TbtData) -> np.ndarray:
    ...
    for index in range(tbt_data.nbunches):
        for plane in PLANES:
            data[PLANE_TO_NUM[plane], :, index, :] = tbt_data.matrices[index][
                plane
            ].to_numpy()
    return data
...
def add_noise(
    ...
    return np.array(
        data + scaling * np.random.default_rng(seed).standard_normal(data.shape)
    )
...
def add_noise_to_tbt(
    ...
    rng = np.random.default_rng(
        seed
    )  # as we don't want to use the same noise on all dataframes
    return TbtData(
        matrices=[
            TransverseData(
                X=pd.DataFrame(
                    index=m.X.index,
                    data=add_noise(m.X.to_numpy(), noise, sigma, seed=rng),
                ),
                Y=pd.DataFrame(
                    index=m.Y.index,
                    data=add_noise(m.Y.to_numpy(), noise, sigma, seed=rng),
                ),
            )
...
def numpy_to_tbt(
    names: np.ndarray, matrix: np.ndarray, datatype: DataType = TransverseData
) -> TbtData:
    """
    ...
    for idx_bunch in range(nbunches):
        matrices.append(
            datatype(  # datatype is directly the class to load data into (TransverseData or TrackingData)
                **{  # for each field in the datatype, load the corresponding matrix
                    field: pd.DataFrame(
                        index=names, data=matrix[idx_field, :, idx_bunch, :]
                    )
