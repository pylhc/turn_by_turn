...
def read_tbt(file_path: str | Path) -> TbtData:
    ...
    matrices = [
        {p: {bpm: np.zeros(n_turns) for bpm in bpms} for p in PLANES}
...
def _read_header(lines: Sequence[str]) -> tuple[datetime, int]:
    ...
    return datetime.strptime(
        f"{date_str[DATE]} {date_str[TIME]}", TIME_FORMAT
    ), idx_line
...
def _read_data(
    ...
    for line in lines:
        ...
        for plane in PLANES:
            matrices[part_id][plane][segment.name][turn_nr] = float(
                data[column_map[plane]]
            )
    return matrices
...
def _parse_column_names_to_indices(parts: Sequence[str]) -> dict:
    ...
    if any(missing):
        raise ValueError(
            f"The following columns are missing in ptc file: '{str(missing)}'"
        )
    return col_idx
