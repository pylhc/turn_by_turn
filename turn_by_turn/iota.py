    ...
    with h5py.File(file_path, "r") as hdf_file:
        ...
        matrices = [
            TransverseData(
                X=pd.DataFrame(
                    index=bpm_names,
                    data=FUNCTIONS[hdf5_version]["get_tbtdata"](
                        hdf_file, "X", hdf5_version
                    ),
                    dtype=float,
                ),
                Y=pd.DataFrame(
...
def _get_turn_by_turn_data_v1(
    hdf5_v1_file: h5py.File, plane: str, version: int
) -> np.ndarray:
...
def _get_turn_by_turn_data_v2(
    hdf5_v2_file: h5py.File, plane: str, version: int
) -> np.ndarray:
