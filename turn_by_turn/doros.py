...
class DataKeys:
    ...
    def get_other_data_keys(cls, data_type: str) -> DataKeys:
        ...
        other_type = (
            cls.POSITIONS if data_type == cls.OSCILLATIONS else cls.OSCILLATIONS
        )
...
def read_tbt(
    file_path: str | Path,
    bunch_id: int = DEFAULT_BUNCH_ID,
    data_type: str = DataKeys.OSCILLATIONS,
) -> TbtData:
    """
    ...
    with h5py.File(file_path, "r") as hdf_file:
        ...
        bpm_names = [
            ...
            if data_keys.n_samples in hdf_file[f"/{name}"]
        ]
        ...
        nturns = hdf_file[bpm_names[0]][data_keys.n_samples][
            0
        ]  # equal lengths checked before
        matrices = [
            TransverseData(
...
def write_tbt(
    file_path: str | Path, tbt_data: TbtData, data_type: str = DataKeys.OSCILLATIONS
) -> None:
    """
    ...
    with h5py.File(file_path, "w", track_order=True) as hdf_file:
        ...
        for bpm in data.X.index:
            ...
            hdf_file[bpm].create_dataset(
                ACQ_STAMP, data=[tbt_data.date.timestamp() * 1e6]
            )
            hdf_file[bpm].create_dataset(
                BST_TIMESTAMP, data=[tbt_data.date.timestamp() * 1e6]
            )

            hdf_file[bpm].create_dataset(data_keys.n_samples, data=[tbt_data.nturns])
            hdf_file[bpm].create_dataset(
                data_keys.data["X"], data=data.X.loc[bpm, :].values
            )
            hdf_file[bpm].create_dataset(
                data_keys.data["Y"], data=data.Y.loc[bpm, :].values
            )

            hdf_file[bpm].create_dataset(other_keys.n_samples, data=0)
            hdf_file[bpm].create_dataset(
                other_keys.data["X"], data=[other_keys.default_value]
            )
            hdf_file[bpm].create_dataset(
                other_keys.data["Y"], data=[other_keys.default_value]
            )


def _create_dataframe(
    hdf_file: h5py.File, data_keys: DataKeys, bpm_names: str, plane: str
) -> pd.DataFrame:
...
def _check_data_lengths(
    hdf_file: h5py.File, data_keys: DataKeys, bpm_names: str
) -> None:
    ...
    for bpm in bpm_names:
        ...
        if n_turns != len(hdf_file[bpm][data_keys.data["X"]]) or n_turns != len(
            hdf_file[bpm][data_keys.data["Y"]]
        ):
    ...
    if not all_elements_equal(
        hdf_file[bpm][data_keys.n_samples][0] for bpm in bpm_names
    ):
