...
def write_tbt(
    ...
    LOGGER.info(
        f"Writing TbTdata in binary SDDS (SPS) format at '{output_path.absolute()}'"
    )
    ...
    if add_trailing_bpm_plane:
        bpm_names_x = [
            f"{bpm_name}.H" if not bpm_name.endswith(".H") else bpm_name
            for bpm_name in bpm_names_x
        ]
        bpm_names_y = [
            f"{bpm_name}.V" if not bpm_name.endswith(".V") else bpm_name
            for bpm_name in bpm_names_y
        ]
