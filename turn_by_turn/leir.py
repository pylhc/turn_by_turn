...
Data handling for turn-by-turn measurement files from the ``LEIR`` (files in **SDDS** format).
"""
import logging
...
def reorder_planes(data, bpm_names):
    for ii, bpm in enumerate(bpm_names):
        if 'V' in bpm:
            data['Y'][ii] = data['X'][ii]
            data['X'][ii] = data['X'][ii]*0
    return data


def read_tbt(file_path: Union[str, Path], reorder_planes=True) -> TbtData:
    """
    Reads turn-by-turn data from the ``LEIR``'s **SDDS** format file.
    ...
    LOGGER.debug(f"Reading LEIR file at path: '{file_path.absolute()}'")
    ...
    if reorder_planes:
        data = reorder_planes(data, bpm_names) # temporary fix
    ...
    return TbtData(matrices, date, bunch_ids, nturns), data


def write_tbt(output_path: Union[str, Path], tbt_data: TbtData) -> None:
    """
    Write a ``TbtData`` object's data to file, in the ``LEIR``'s **SDDS** format.
    ...
    LOGGER.info(f"Writing TbTdata in binary SDDS (LEIR) format at '{output_path.absolute()}'")
